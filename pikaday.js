...
}(this, function (moment)
{
    ...
    renderDay = function(opts)
    {
        ...
        if (opts.isEmpty) {
            if (opts.showOffMonthDays) {
                arr.push('is-off-month');
            } else {
        ...
        self._onMouseDown = function(e)
        {
            ...
            if (!hasClass(target.parentNode, 'is-disabled')) {
                if ((hasClass(target, 'pika-button') && !hasClass(target, 'is-empty'))
                    || (opts.showToday && (isToday = hasClass(target, 'pika-today-title')))) {
                    self.setDate(isToday ? new Date() :
                        new Date(target.getAttribute('data-pika-year'), target.getAttribute('data-pika-month'), target.getAttribute('data-pika-day')));
        ...
        render: function(year, month)
        {
            ...
            for (var i = 0, r = 0; i < cells; i++)
            {
                ...
                dayConfig = {
                    ...
                    isInRange: isInRange,
                    showOffMonthDays: opts.showOffMonthDays
                };
